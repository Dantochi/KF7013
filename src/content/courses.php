...
$conn = getConnection();
$sql = "SELECT * FROM training_sessions"; // This query gets all the details from the training_sessions table
...
if($result){
    ...
    while($row = mysqli_fetch_assoc($result)){ // if the associative array is not empty, the while loop runs
        $id = $row['trainingID'];
        $title = $row["title"];
        $description = $row["description"];
        $session_date = strrev(substr($row["session_date"], 0, 10));
