...
if(isset($_REQUEST["email"])){
    $emailAddress = filter_var($_REQUEST["email"], FILTER_SANITIZE_EMAIL); // This receives the details from the form submission on the login.html page via POST request
    $password = $_REQUEST["password"];
    $sql = "SELECT * FROM customers WHERE customer_email = ?"; // To avoid processing malicious script, we use prepared statements
    $conn = getConnection();
    $stmt = mysqli_prepare($conn, $sql);
    if($stmt){ // If the condition is true, the form input would be processed, and the query to the database is executed
        mysqli_stmt_bind_param($stmt, "s",  $emailAddress);
        mysqli_stmt_execute($stmt);
        $result = mysqli_stmt_get_result($stmt);
        $row = mysqli_fetch_assoc($result);
        if($result == null){ // If the result has a value of null, an error message is outputted
            echo "You have inputted a wrong email or password";
        } else { // If the query command was executed, details are gotten from the customers table
